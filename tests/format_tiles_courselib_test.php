...
class format_tiles_course_courselib_testcase extends advanced_testcase {
    ...
    private function assign_create_run_asserts($moduleinfo, $dbmodinstance) {
        ...
        // The goal not being to fully test assign_add_instance() we'll stop here for the assign tests.
        // (To avoid too many DB queries).
    ...
    private function create_specific_module_test($modulename) {
        ...
        $moduleinfo->conditionfieldgroup = array(
            array(
                ...
                'conditionfieldoperator' => \availability_profile\condition::OP_CONTAINS,
                'conditionfieldvalue' => '@'
            )
        );
        $moduleinfo->conditioncompletiongroup = array(
            array('conditionsourcecmid' => $assigncm->id, 'conditionrequiredcompletion' => COMPLETION_COMPLETE)
        );
        ...
        $moduleinfo->introeditor = array('text' => 'This is a module', 'format' => FORMAT_HTML, 'itemid' => $draftideditor);
...
//            $this->assertEquals($value, $newcm->$prop);
            // In Moodle 3.6 (copy) will be added to the copy name
            $this->assertTrue($value == $newcm->$prop || ($prop === 'name' && $value . ' (copy)' === $newcm->$prop));
        }
    }
