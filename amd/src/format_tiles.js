...
define(["jquery", "core/templates", "core/ajax", "format_tiles/browser_storage",
    ...
    var Selector = {
        ...
        // We try several different selectors for header bar as it varies between theme (boost based, clean based, essential etc)
    };
        ...
        $("ul.tiles").children(Selector.TILE).not($(Selector.TILE_COLLAPSED)).each(function (index, tile) {
            ...
            if (currentSectionId) {
                if (index === 0) {
                    ...
                    // The reference to maxVerticalPositionDifference is because one tile
                    // may have a slightly different vertical position from another on the *same* row
                ...
                previousTile = tile;
                // Add displayAfterTile value to the current row so that is ends up showing the value of the last tile in this row
    ...
    var setCourseContentHTML = function (contentArea, content) {
        if (content) {
            ...
            if (contentArea.attr("id") !== Selector.SECTION_ZERO) {
                /**
                 * Trap the tab key navigation in the content bearing section until the user
                 * clicks the close button - when user reaches last item, send them back to first
                ...
                if (!scrollFuncLock && sectionIsOpen) {
                    ...
                    setTimeout(function () {
                        ...
                        if (desiredNewPositionInSection > 0
                            ...
                            // var newScrolltopDown = $(window).scrollTop() - contentArea.outerHeight();
    ...
    var failedLoadSectionNotify = function(sectionNum, failResult, contentArea) {
        if (failResult) {
            ...
            // It looks like we may not have a connection so we can't
            // launch notifications, but we can warn the user like this instead
            ...
            $(document).ready(function () {
                ...
                if (useJavascriptNav && !isEditing) {
                    ...
                    */
                    ...
                    /**
                     * If user clicks the window overlay behind the visible tile content
                     * they want to remove the overlay and deselect the tile
                     */
                    ...
                    pageContent.on(Event.CLICK, Selector.TILE, function (e) {
                        ...
                        if (thisTile.hasClass(ClassNames.SELECTED)) {
                            ...
                            ajax.call([{methodname: "format_tiles_log_tile_click", args: {
                                    courseid: courseId,
                    ...
                    /**
                     * When the window is re-sized, the content sections under the tiles may then
                     * be in the wrong place, so remove them and re-initialise them.
                     * Collapse the selected section before doing this otherwise the re-orgnisation
                     * wont work as the tiles' flow will be out when they are analysed
                     */
                    $(window).on("resize", function () {
                        // On iOS resize events are triggered often on scroll because the address bar hides itself, so avoid this
                    ...
                    /**
                     * If the nav drawer is opened or closed, this also rezises the window so need
                     * to re-initialise content divs
                     */
                     ...
                     * to it, which we can show when the main overlay is active
                     * When a user clicks this overlay, they want to close the overlay and click
                     * the menu item behind so provide for that here
                    ...
                    if (headerBar.outerHeight() !== undefined) {
                        headerOverlay = $("<div></div>").addClass(ClassNames.HEADER_OVERLAY).attr("id", ClassNames.HEADER_OVERLAY)
                    ...
                    /**
                     * When we first load the page we want to move the tile contents divs so that they
                     * are in the correct rows according to which row of tiles they relate to
                     */
                ...
                // If theme is displaying the .tiles_coursenav class items, as attempted to be added in lib.php
                // extend_course_navigation, show items with this class as they will be hidden otherwise
                ...
                /**
                 * Get these strings now, in case we need them e.g. after we lose connection and cannot display
                 * content on a user tile click
                 */
                str.get_strings([
