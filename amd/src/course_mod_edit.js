    ...
    var toggleHideCourseMod = function (clickEvent) {
        ...
        if (actions) {
            ...
            promises[0].done(function (HTMLresult) {
                // This core web service returns the new HTML for the course module, but
                // we don't want to use it, as we are using different templates for the CM
                // to save re-implementing the core function, we replace elements in the old
                // HTML, instead of generating new HTML.
                ...
                clickedLink.attr("href", clickedLink.attr("href").replace("&" + actions.old + "=", "&" + actions.changeTo + "="));
    ...
    return {
        init: function (convertedLabel) {
            $(document).ready(function () {
                 ...
                 * which is called on each drag drop and results in tbe course module which is drag dropped
                 * being refreshed afterwards, changing  it in the process to a non tiles format.  This stops that.
                ...
                $(document).on(Event.MODULE_ADDED, function (event, msg) {
                    ...
                    if (addedCourseModule.hasClass(ClassNames.ACTIVITY) && addedCourseModule.closest('ul').hasClass('subtiles')) {
                        ...
                        // and  re-render it in the correct style for this format (as sub tile)
                            ...
                            str.get_string(stringKey, "format_tiles")
                                .done(function (string) {
                                    ...
                                    Templates.render(
                                        ...
                                        // Notification.alert(stringStore.complete, stringStore.fileaddedtobottom);
                                    });
                                });
                        } else {
                ...
                /**
                 * If the teacher deletes a subtile, we need to hide any "spacer" subtiles
                 * which appear directly before it, so tht there are none between what was
                 * the previous activity to the one we just deleted, and the next
                 */
